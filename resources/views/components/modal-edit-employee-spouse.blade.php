<div id="spouseModal{{ $spouse->id }}" class="fixed z-10 inset-0 overflow-y-auto hidden">
    <div class="fixed inset-0 flex items-center justify-center">
        ...
        <div class="relative z-10 bg-white rounded-lg shadow-xl max-w-3xl mx-auto p-6">
            <form action="{{ route('employee.spouse.update', [$employee->employee_number, $spouse->id]) }}" method="POST"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" id="birth_date" name="birth_date"
                            value="{{ old('birth_date', $spouse->birth_date->format('Y-m-d')) }}"
                            class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        @error('birth_date')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    ...
                    <div>
                        ...
                        <select id="allowance_status" name="allowance_status"
                            class="form-select rounded-md shadow-sm mt-1 block w-full">
                            <option value="eligible" {{ $spouse->allowance_status == 'eligible' ? 'selected' : '' }}>
                                Berhak</option>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                    <div>
                        ...
                        <input type="date" id="marriage_date" name="marriage_date"
                            value="{{ old('marriage_date', $spouse->marriage_date->format('Y-m-d')) }}"
                            class="form-input rounded-md shadow-sm mt-1 block w-full" />
                        @error('marriage_date')
                            <span class="text-red-500 text-sm">{{ $message }}</span>
                        @enderror
